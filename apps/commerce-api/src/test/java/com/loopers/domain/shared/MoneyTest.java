...
class MoneyTest {
    ...
    class Create {
        ...
        @DisplayName("금액이 null이면 Money 객체 생성에 실패한다.")
        @ParameterizedTest
        @NullSource
        void throwsIllegalArgumentException_whenValueIsInvalid(Long invalidValue) {
            assertThatThrownBy(() -> Money.of(invalidValue))
    ...
    class Add {
        ...
        void throwsIllegalArgumentException_whenOverflow() {
            var money1 = Money.of(Long.MAX_VALUE);
            var money2 = Money.of(1L);
        ...
        void success_whenAddingValidAmounts() {
            var money1 = MoneyFixture.builder().build();
            var money2 = MoneyFixture.builder().build();
        ...
        void success_whenAddingWithZero() {
            var money = MoneyFixture.builder().build();
            var zero = MoneyFixture.builder().amount(0).build();

            var result1 = money.add(zero);
            var result2 = zero.add(money);
    ...
    class Subtract {
        ...
        void throwsIllegalArgumentException_whenResultWouldBeNegative(Long minuend, Long subtrahend) {
            var money1 = MoneyFixture.builder().amount(minuend).build();
            var money2 = MoneyFixture.builder().amount(subtrahend).build();
