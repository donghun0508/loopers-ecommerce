...
class ProductManagerTest {
    ...
    void throwsExceptionWhenProductListIsNullOrEmpty(List<Product> invalidProductList) {
        ...
        assertThatThrownBy(() -> ProductManager.assign(invalidProductList, stockMap))
    ...
    void throwsExceptionWhenStockMapIsNullOrEmpty(Map<Long, Stock> invalidStockMap) {
        ...
        assertThatThrownBy(() -> ProductManager.assign(products, invalidStockMap))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("ProductManager.assign().purchaseProducts");
    }

    @DisplayName("상품 관리자 할당 시, 상품 ID와 구매할 상품 ID가 일치하지 않는 경우 예외가 발생한다.")
    @Test
    void throwsExceptionWhenProductIdDoesNotMatch() {
        ...
        ProductManager productManager = ProductManager.assign(products, stockMap);

        assertThatThrownBy(productManager::decreaseStock)
